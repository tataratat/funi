...
inline void Uff(DataType* original_points,          /* in */
  ...
  for (IndexingType i{0}; i < number_of_points; i++) {
    vector_metric[i] =
        metric[0] * original_points[i * point_dim];
    for (IndexingType j{1}; j < point_dim; j++) {
      vector_metric[i] +=
          metric[j] * original_points[i * point_dim + j];
    }
  }
  ...
  std::vector<bool> newpointmasks(
      number_of_points);  // zero (false) init
  ...
  for (IndexingType lower_limit{0};
    ...
    if (!stable) {
      for (IndexingType i_dim{0}; i_dim < point_dim; i_dim++) {
        new_points[number_of_new_points * point_dim +
                            i_dim] =
            original_points[metric_order_indices[lower_limit] *
                                         point_dim +
                                     i_dim];
        new_indices[number_of_new_points] =
  ...
  if (inverse[metric_order_indices[last_index]] == -1) {
    if (!stable) {
      for (IndexingType i_dim{0}; i_dim < point_dim; i_dim++) {
        new_points[number_of_new_points * point_dim +
                            i_dim] =
            original_points[metric_order_indices[last_index] *
                                         point_dim +
                                     i_dim];
        new_indices[number_of_new_points] =
